...
public interface TransferEntryRepository extends EntryRepository<TransferEntry, EquityAccount, EquityCategory, EquityAccount, EquityCategory> {
	...
	@Query("SELECT SUM(e.value) as value FROM Entry e WHERE e.inOwner = ?1 and e.inAccount = ?2")
